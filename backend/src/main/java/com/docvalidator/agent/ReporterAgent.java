...
public class ReporterAgent {
    ...
    public ReporterAgent(DocValidatorConfig config) {
        this.config = config;
        ...
        if ("openai".equalsIgnoreCase(provider) && realKey) {
            ...
            log.info("OpenAI service initialized for report generation");
        } else {
            this.openAiService = null;
            log.info("OpenAI service skipped (provider={}, key configured={}). AI recommendations disabled.", provider, realKey);
        }
    }
    ...
    public ValidationReport generateReport(List<ValidationResult> validationResults) {
        ...
        Map<Discrepancy.DiscrepancyType, List<Discrepancy>> discrepanciesByType = 
                groupDiscrepanciesByType(validationResults);
        
        Map<Discrepancy.Severity, List<Discrepancy>> discrepanciesBySeverity = 
                groupDiscrepanciesBySeverity(validationResults);
        
        // Generate AI recommendations for top issues
        List<ValidationReport.Recommendation> recommendations = 
                generateRecommendations(discrepanciesByType, discrepanciesBySeverity);
    ...
    private List<ValidationReport.Recommendation> generateRecommendations(
        ...
        for (Discrepancy discrepancy : topIssues) {
            try {
                ValidationReport.Recommendation recommendation = 
                        generateRecommendationForDiscrepancy(discrepancy);
        ...
        if (!byType.isEmpty()) {
            ValidationReport.Recommendation overallRecommendation = 
                    generateOverallRecommendation(byType, bySeverity);
    ...
    private ValidationReport.Recommendation generateRecommendationForDiscrepancy(
        ...
        String aiResponse = "Configure an OpenAI API key to get AI-generated recommendations.";
        if (openAiService != null) {
            try {
                ...
                ChatCompletionRequest request = ChatCompletionRequest.builder()
                        ...
                        .maxTokens(300)
                        .build();
                aiResponse = openAiService.createChatCompletion(request)
    ...
    private ValidationReport.Recommendation generateOverallRecommendation(
        ...
        String aiResponse = "Configure an OpenAI API key to get AI-generated strategic recommendations.";
        if (openAiService != null) {
            try {
    ...
    private String buildOverallPrompt(
        ...
        byType.forEach((type, discrepancies) -> 
                prompt.append(String.format("- %s: %d issues\n", type, discrepancies.size())));
        
        prompt.append("\nDiscrepancies by Severity:\n");
        bySeverity.forEach((severity, discrepancies) -> 
